...
module axi_to_detailed_mem #(
  ...
  localparam int unsigned mem_data_width = DataWidth / NumBanks,
  localparam type mem_data_t = logic [mem_data_width-1:0],
  /// Dependent parameter, do not override. Memory write strobe type.
  localparam int unsigned mem_strb_width = DataWidth / NumBanks / 8,
  localparam type mem_strb_t = logic [mem_strb_width-1:0],
...
module axi_to_detailed_mem_intf #(
  ...
  localparam int unsigned mem_data_width = DATA_WIDTH / NUM_BANKS,
  localparam type mem_data_t = logic [mem_data_width-1:0],
  /// Dependent parameter, do not override. See `axi_to_detailed_mem`, parameter `mem_strb_t`.
  localparam int unsigned mem_strb_width = DATA_WIDTH / NUM_BANKS / 8,
  localparam type mem_strb_t = logic [mem_strb_width-1:0],
...
module mem_stream_to_banks_detailed #(
  ...
  localparam int unsigned OupStrbWidth = DataWidth / NumBanks / 8,
  ...
  output logic       [OupDataWidth-1:0] [NumBanks-1:0] bank_wdata_o,
  /// Memory bank request, request write strobe.
  output logic       [OupStrbWidth-1:0] [NumBanks-1:0] bank_strb_o,
  ...
  input  logic       [OupDataWidth-1:0] [NumBanks-1:0] bank_rdata_i,
  ...
  localparam int unsigned BitsPerBank  = OupDataWidth;
  localparam int unsigned BytesPerBank = OupStrbWidth;

  typedef struct packed {
    addr_t     addr;
    logic [OupDataWidth-1:0] wdata;
    logic [OupStrbWidth-1:0] strb;
    wuser_t    wuser;
    logic      we;
  } req_t;
  ...
  for (genvar i = 0; unsigned'(i) < NumBanks; i++) begin : gen_reqs
    ...
    for (genvar j = 0; j < BitsPerBank; j++) begin
      assign bank_req[i].wdata[j] = wdata_i[i * BitsPerBank + j];
    end
    for (genvar j = 0; j < BytesPerBank; j++) begin
      assign bank_req[i].strb[j] = strb_i[i * BytesPerBank + j];
    end
